...
void Enemy::Update(float dt)
{
	...
	if (scene->GetActor<Player>())
	{
		...
		if (fireTimer <= 0 && angle <= pbls::DegToRad(10))
		{
			...
			std::unique_ptr<Projectile> projectile = (std::make_unique<Projectile>(t, scene->engine->Get<pbls::ResourceSystem>()->Get<pbls::Shape>("Bullet.txt"), 350));
			projectile->transform.scale = 0.5;
